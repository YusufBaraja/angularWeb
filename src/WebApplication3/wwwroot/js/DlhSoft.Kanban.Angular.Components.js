                ...
                .directive('dsKanbanBoard', function () {
                    return {
                        ...
                        controller: function ($scope,itemservice) {
                            ...
                            this.onItemDrop = function (type, index, group, state, targetIndex,assignableResources) {
                                debugger;
                                if (type !== KanbanBoard.types.item || targetIndex === index)
                                    return;
                                ...
                                //additional code for handling drag and drop update
                                var updateItem = {
                                    Id: item.newItemId,
                                    Name:item.name,
                                    GroupId: group.newGroupId,
                                    StateId: state.newStateId,
                                    ResourceId: item.assignedResource.newResourceId
                                  //  resourceId:item.
                                };
                                itemservice.update(item.newItemId, updateItem);
                                hub.server.send();
                            ...
                            this.addNewItem = function (group, state) {
                                ...
                                var item = { name: _this.newItemName, group: group, state: state, assignedResource: _this.newItemResource
                                    , groupId: group.newGroupId, stateId: state.newStateId, resourceId: 1,

                                };
                                var item2 = {
                                    name: _this.newItemName, groupId: group.newGroupId, stateId: state.newStateId, resourceId: 1
                                }
                               // _this.items.push(item);
                                if (_this.onAddingNewItem != null) {
                                    ...
                                    var saveItemRecord = itemservice.save(item);
                                  
                                    saveItemRecord.then(function (d) {                                       
                                       hub.server.addItem(d.data);
                                   },
                                function () {
                                    alert("no");
                                });
                            ...
                            //this.editGroupButtonText = function (group) {
                            //    debugger;
                            //    var x = {
                            //        groupId: group.newGroupId, group: group
                            //    };
                            //}
                        },
                        controllerAs: 'dskb',
